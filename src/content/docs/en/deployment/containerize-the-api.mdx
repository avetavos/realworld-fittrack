...
- **Pros:** `ghcr.io/astral-sh/uv:python3.12-bookworm-slim` ships a known-good `uv` and the matching Python already assembled, so the builder stage starts installing immediately with no bootstrap step; the version of `uv` is pinned by the tag, which keeps builds reproducible.
...
- Why does the runtime stage start from `python:3.12-slim-bookworm` instead of the `uv` image the builder used? What is deliberately *not* in the final image?
