...
`asyncio_mode = "auto"` tells `pytest-asyncio` to treat every `async def test_*` as a coroutine test — no `@pytest.mark.asyncio` on each function. `testpaths` keeps collection pointed at the `tests/` directory. The two `loop_scope` lines matter as soon as a fixture is session-scoped (the `engine` fixture below is): pytest-asyncio 1.x defaults every test *and* every fixture to its own function-scoped event loop, so a session-scoped engine's pooled connections get created in the first test's loop and then explode with `RuntimeError: ... attached to a different loop` the moment a later test — running in its own, different loop — tries to reuse them. Setting both scopes to `"session"` puts the engine, every fixture that depends on it, and every test in the *same* event loop for the whole run, which is what a shared, pooled connection actually requires.
...
- The `session` fixture inserts a row into `auth.users` for `FAKE_USER_ID` with `flush()` before yielding, and lets the `on_auth_user_created` trigger create the matching `profiles` row. Why does the profile have to exist before any workout can be logged, why insert into `auth.users` rather than `profiles` directly, and why `flush()` rather than `commit()`?
