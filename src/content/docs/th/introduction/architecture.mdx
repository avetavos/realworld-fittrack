...
client ทั้งสอง **ยืนยันตัวตนด้วย Supabase Auth** แล้วได้ JWT กลับมา จากนั้นทุก request ที่ยิงไป backend จะพก token นั้นไปด้วย และ **FastAPI เป็นสิ่งเดียวที่แตะ data** — backend เป็นคน verify JWT, รัน business logic แล้วอ่านและเขียน Supabase Postgres ผ่าน connection แบบ async client ไม่เคย query database ตรง ๆ เลย

## ทำไมต้องแบ่งแบบนี้

Supabase ใช้เป็น backend-as-a-service ล้วน ๆ ก็ได้ — client คุยตรงไปยัง Postgres ผ่าน Row-Level Security แต่ FitTrack ตั้งใจไม่ทำแบบนั้น เราวาง **FastAPI ไว้ตรงกลาง** ด้วยสามเหตุผล:
