...
description: "web/ — SvelteKit app ที่ scaffold ด้วย `sv create` แล้ว sign in ผ่าน Supabase ด้วย @supabase/supabase-js หน้า sign-in เรียก signInWithPassword, sync session ของ browser ไปที่ cookie, hooks.server verify token เข้า locals, และ +layout.server.ts ส่ง session ให้ทุก route env มาจาก $env ตรวจสอบด้วยการ sign in แล้วเห็น session ทั้งฝั่ง client และ server"
sidebar:
  order: 1
---
...
- **Cons:** คุณรับ session model ของ Supabase มาทั้งชุด รวมถึงเรื่อง browser-vs-server ที่บทนี้ต้องเขียนโค้ดอ้อมไปแก้ และคุณผูกกับ SDK และ token lifecycle ของ Supabase แทนที่จะเป็นเจ้าของเอง สำหรับ companion client ที่ควรทำตัวเหมือน client หลักเป๊ะ ๆ การใช้ auth system ร่วมกันคุ้มค่าชัดเจน

**Syncing the session to a cookie for the server** vs. **`@supabase/ssr` with full server-side auth**

- **Pros:** วิธี `supabase-js` + cookie ล้วน ๆ นั้นเล็กและโปร่งใส — คุณเห็นชัดว่า token ถูก set, verify, และ forward ที่ไหน ซึ่งเหมาะกับ companion ที่เบาและเน้นอ่าน วิธีนี้ปล่อยให้ `supabase-js` ทำสิ่งที่ถนัดใน browser และขอให้ server แค่ verify token ที่ได้รับมา
- **Cons:** นี่คือ bridge ที่ทำเองด้วยมือ ไม่ใช่ตัวที่ framework รับรอง — `@supabase/ssr` จัดการ cookie, refresh, และ SSR session hydration ให้คุณ และเป็นตัวเลือกที่ถูกต้องสำหรับ web app ที่มี auth เยอะ ฝั่ง web ของ FitTrack อ่านมากกว่าเขียน surface ที่เล็กกว่าจึงชนะที่นี่ หยิบ `@supabase/ssr` มาใช้เมื่อ web client โตขึ้นจนมี write flow จริงจัง

## ติดตั้ง

### 1. Scaffold `web/` ด้วย Svelte CLI
...
web client ต้องใช้ Supabase project URL และ **anon** (public) key — ค่าเดียวกับที่ Flutter app ใช้ และเป็นค่าเดียวกับใน Module 1 prefix `PUBLIC_` คือสิ่งที่ทำให้ `$env/static/public` expose ค่าพวกนี้ไป browser ได้:
...
Supabase client ฝั่ง browser ตัวเดียวที่ทั้ง app ใช้ร่วมกัน อ่าน config จาก `$env/static/public` ดังนั้นชื่อตัวแปรถูกตรวจตอน build:
...
หน้า sign-in เรียก `signInWithPassword` และเมื่อสำเร็จก็เขียน access token ลง cookie เพื่อให้ server เห็น session แล้วพากลับหน้า home (syntax runes ของ Svelte 5 เปลี่ยน `$state` เป็น `let` ถ้าคุณอยู่บน Svelte 4):
...
`hooks.server.ts` รันทุก server request โดยอ่าน cookie, ขอให้ Supabase verify token (เช็ค signature + expiry จริง ไม่ใช่เชื่อแบบมืด ๆ) แล้ววาง session ที่ได้ลงบน `event.locals` ให้ request ที่เหลือใช้:
...
บอก TypeScript ว่าเราวางอะไรไว้บน `locals` ด้วยการ declare type ใน `src/app.d.ts`:
...
`+layout.server.ts` `load` ที่ root รันบน server ให้ทุก route แล้ว return session เพื่อให้ทุกหน้าและทุก child `load` ได้รับต่อ เพราะอยู่ใน layout ระดับ app ทั้งหมด จึงใช้ source of truth เดียวสำหรับ "ใคร sign in อยู่":
...
ใช้ session ใน root layout เพื่อแสดง auth state และ link ไปหน้า sign-in:
...
ตรวจให้แน่ใจว่า Supabase ตัว local ของคุณรันอยู่ (จาก Module 2) และคุณมี user ให้ sign in ถ้ายังไม่มี สร้างด้วย Supabase CLI:
...
เปิด [http://localhost:5173/signin](http://localhost:5173/signin), ใส่ email และ password ที่ถูกต้อง แล้ว submit เมื่อสำเร็จระบบจะ redirect กลับ home และ nav แสดง **Signed in as …** — พิสูจน์ว่า session ฝั่ง browser มีอยู่ ทีนี้มาพิสูจน์ว่า *server* เห็น session ด้วย: reload หน้า (ครบรอบ server เต็ม ๆ) แล้ว nav ยังแสดง email ของคุณ เพราะ `hooks.server.ts` อ่าน cookie และ `+layout.server.ts` return session ยืนยันว่า cookie อยู่ครบใน browser devtools ใต้ Application → Cookies:
...
- `supabase.auth.getUser(token)` ใน `hooks.server.ts` ตรวจอะไรจริง ๆ และทำไมถึงแข็งแรงกว่าแค่ decode payload ของ token?
- ทำไม Supabase URL และ anon key ถึงมาจาก `$env/static/public` พร้อม prefix `PUBLIC_` ส่วน JWT secret ต้องไม่ปรากฏใน project นี้เลย?
