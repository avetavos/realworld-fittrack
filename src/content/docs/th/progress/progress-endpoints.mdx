...
เพิ่ม per-exercise trend ให้ `ProgressRepo` จาก [บทที่แล้ว](/fittrack/th/progress/aggregation-queries/) — หนึ่งแถวต่อ session สำหรับ exercise ตัวเดียว เรียงเก่าสุดไปใหม่สุดเพื่อให้ client plot จากซ้ายไปขวาได้
...
เก่าสุดก่อน เพื่อให้ chart อ่านจากซ้ายไปขวาและเห็น trend ขาขึ้น เป็นการเช็คสุดท้ายว่า backend ทั้งหมดต่อกันติด เปิด [http://localhost:8000/docs](http://localhost:8000/docs) — ทุก route จากทั้งสาม module (exercises, workouts, progress) ถูก list พร้อม request และ response schema ครบถ้วน เป็น API contract เต็มที่ client จะ build ทับ
