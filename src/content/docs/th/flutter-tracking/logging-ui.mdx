...
description: "สร้าง flow หลัก: เริ่ม workout, เลือก exercise จาก GET /exercises, เพิ่ม set (reps + weight_kg) แล้ว save ทั้ง session ในครั้งเดียวด้วย POST /workouts Riverpod Notifier ถือ draft ที่กำลังทำอยู่เป็น immutable state; หน้าจอ watch draft นั้นและ exercise catalog ทำไมถึงใช้ draft ฝั่ง client และ save แบบ transactional ครั้งเดียว ตรวจสอบด้วยการ log workout สองเซ็ตแล้วเห็นข้อมูล persist ผ่าน API"
sidebar:
  order: 1
---
...
หัวใจของ FitTrack: หน้าจอที่ผู้ใช้ **log workout** ผู้ใช้เริ่ม session, **เลือก exercise** จาก catalog (`GET /exercises`), **เพิ่ม set** — แต่ละอันเป็นจำนวน `reps` กับ `weight_kg` — ก่อรายการขึ้นมา แล้ว **save ทั้งหมดในครั้งเดียว** ด้วย `POST /workouts` ตัวเดียว ตรงตามที่ [Workouts API →](/fittrack/th/workouts/) คาดไว้ (workout และ set ทั้งหมดสร้างขึ้นใน transaction เดียว) workout ที่กำลังทำอยู่อยู่ใน Riverpod **`Notifier`** เป็น immutable state; หน้าจอ watch draft นั้นและ exercise catalog แล้วเรียก [API client →](/fittrack/th/flutter-foundation/api-client/) จาก module ที่แล้วเพื่อ persist draft
...
- **Cons:** มีโครงสร้างมากกว่าหน้าจอที่ขับด้วย `setState` ตัวเดียว; และคุณต้องมีวินัยว่าจะแทนที่ state เสมอ ไม่ mutate ของเดิม เพราะการ log กินพื้นที่ทั้ง picker และ list และต้องไม่หายตอน navigation `Notifier` จึงคุ้ม

## ติดตั้ง

### 1. Model — `lib/src/features/workouts/models.dart`
...
### 2. catalog provider — `lib/src/features/workouts/exercises_provider.dart`
...
### 3. draft notifier — `lib/src/features/workouts/logging_controller.dart`
...
### 4. ตัวหน้าจอ — `lib/src/features/workouts/log_workout_screen.dart`
...
### 5. ทำ route เข้าหน้านี้ — แก้ `lib/src/router.dart`

เพิ่ม route `/log` และปุ่มบน home screen เพื่อเข้าหน้านั้น:
...
sign in แตะ **Log a workout** แล้ว **Add set**: เลือก exercise กรอก `5` reps และ `60` kg แล้ว **Add** ทำซ้ำสำหรับ set ที่สอง list โชว์ทั้งคู่ แตะ **Save workout** — snackbar อ่านว่า **Workout saved** และ draft ถูก clear ยืนยันว่าข้อมูล persist จริงด้วยการถาม history ที่ save เพิ่งสร้างจาก backend:
...
- `WorkoutDraft.toJson` กำหนด `set_index` จากตำแหน่งใน list index นั้นมาจากไหนในโค้ด และทำไม backend ถึงอยากได้ค่านั้น?
- catalog เป็น `exercisesProvider` (ตัว `FutureProvider`) provider ตัวนี้ให้ UI state สองอย่างอะไรฟรี ๆ ที่การ fetch เองจะทำให้คุณต้องตามจดเอง?
