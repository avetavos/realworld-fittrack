...
พอจบบท workout ที่คุณ log บทที่แล้วปรากฏใน history พร้อม set ครบ และหน้า progress โชว์ท่าที่ยกได้ดีที่สุดกับ breakdown แบบ weekly volume — ทั้งหมดอ่านตรงจาก FastAPI backend ผ่าน [API client →](/fittrack/th/flutter-foundation/api-client/) ตัวเดียวกัน โดยไม่มีการ aggregate ฝั่ง client เลย
...
### 1. Read model — ขยาย `lib/src/features/workouts/models.dart`
...
### 2. Read provider — `lib/src/features/workouts/history_providers.dart`
...
### 3. หน้า history — `lib/src/features/workouts/history_screen.dart`
...
### 4. หน้า progress — `lib/src/features/workouts/progress_screen.dart`
...
### 5. ทำ route ไปทั้งสองหน้า — แก้ `lib/src/router.dart` และ home screen
