...
description: "FitTrack monorepo: api/ (FastAPI), mobile/ (Flutter), web/ (Svelte) และ .env ตัว shared — repository เดียวถือทั้ง backend และ client ทั้งสอง เพื่อให้ API contract กับ app ที่เรียกใช้ evolve ไปด้วยกัน ทำไมถึงเลือก monorepo ตรงนี้ แต่ละ top-level directory ถือครองอะไร และ .env ตัวเดียวที่ป้อน config ให้ทุก service ตรวจสอบด้วย tree และ git status ที่ ignore ของที่ถูกต้อง"
sidebar:
  order: 2
---
...
- **Cons:** frontend กับ backend จริง ๆ ไม่ได้ต้องการ variable *ตัวเดียวกัน* (client ต้องการ Supabase URL กับ anon key; backend ต้องการ database URL กับ JWT secret) ดังนั้นไฟล์เดียวจึงปนเรื่องกัน และคุณต้องระวังว่า client build จะฝังเฉพาะค่า *public* เท่านั้น — ไม่เอา secret ของ backend มาเด็ดขาด FitTrack เก็บทุกค่าไว้ในไฟล์ที่ documented ไฟล์เดียวแต่บอกชัดใน [Auth (Supabase JWT) →](/fittrack/th/auth/) ว่าค่าไหน ship ไป client ได้ปลอดภัยและค่าไหนต้องอยู่ฝั่ง server

## ติดตั้ง

### 1. สร้าง directory ของ client
...
ทั้งสองจะว่างไปจนถึง [Flutter — Foundation →](/fittrack/th/flutter-foundation/) และ [Svelte Web Companion →](/fittrack/th/svelte-companion/); สร้างตอนนี้แค่เพื่อตรึงโครงสร้างไว้
