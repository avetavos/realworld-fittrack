...
ถ้าเครื่องยังไม่มี interpreter ตัวนั้น `uv` ดาวน์โหลดมาให้เอง ไม่ต้องพึ่ง `pyenv` หรือ package manager ของระบบ:

```bash
uv python install 3.14
```

การ pin จะเขียนไฟล์ `.python-version` ไว้ และทุกครั้งที่ `uv run` / `uv sync` ต่อจากนี้ก็จะยึดตามนั้น เพื่อนร่วมทีมที่ clone repo ไปจึงได้ interpreter ตัวเดียวกับคุณ FitTrack เล็งไปที่ **Python 3.14** (เป็นเวอร์ชันปัจจุบันตั้งแต่ตุลาคม 2025) โค้ด backend ในคอร์สนี้รันบน 3.12+ ได้ แต่บางเรื่องที่จะพูดถึงระหว่างทาง — annotation ที่ไม่ถูก evaluate ทันที, `except A, B:` ที่ไม่ต้องใส่วงเล็บ — เป็นพฤติกรรมของ 3.14
