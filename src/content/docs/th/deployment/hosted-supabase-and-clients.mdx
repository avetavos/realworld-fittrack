...
ขั้นทางเทคนิคขั้นสุดท้าย: ทุกอย่างที่เคยรันบน laptop ของคุณตอนนี้รันบน cloud ใน [Containerize the API →](/fittrack/th/deployment/containerize-the-api/) คุณเปลี่ยน `api/` ให้เป็น image ที่ deploy ได้ ตรงนี้คุณชี้มันไปที่ **Supabase project แบบ hosted** แทน stack ในเครื่อง, deploy image ขึ้น host จริง, แล้ว ship client ทั้งสองให้ยิงมาที่ backend สาธารณะตัวนั้น
...
2. **Deploy the API container** ขึ้น Fly.io โดยป้อน `DATABASE_URL` กับ `SUPABASE_JWT_SECRET` แบบ hosted ให้มันเป็น secrets
...
- **Pros of Fly.io:** มัน deploy Docker image ของคุณตรง ๆ, `fly launch` สร้าง `fly.toml` ที่ใช้ได้จาก `Dockerfile` ที่คุณมีอยู่แล้ว, secrets กับ public HTTPS URL อย่างละ command เดียว, และมันรันใกล้ Supabase region ของคุณเพื่อให้ DB latency ต่ำ
...
สร้าง project ใน [Supabase dashboard](https://supabase.com/dashboard) แล้ว link local repo ของคุณเข้ากับมันและ push migration รันคำสั่งเหล่านี้จาก `api/` (หรือที่ไหนก็ตามที่ directory `supabase/` ของคุณอยู่):
...
ทีนี้ set production secrets ค่ามาจาก Supabase dashboard: **Project Settings → Database** ให้ connection string, และ **Project Settings → API → JWT Secret** ให้ signing secret ที่ [`get_current_user` dependency](/fittrack/th/auth/verify-the-jwt/) ของคุณ verify กับมัน สลับ driver เป็น `postgresql+asyncpg` (สิ่งที่ SQLAlchemy คาดหวังจาก [Async database →](/fittrack/th/fastapi/async-database/)) และใช้ connection pooler host เพื่อ deploy ที่เป็นมิตรกับ serverless:
...
ชี้ app ไปที่ API ที่ deploy แล้ว API base URL เป็น compile-time constant ใน [the Flutter API client](/fittrack/th/flutter-foundation/api-client/) — ส่งมันด้วย `--dart-define` เพื่อให้ release build ยิงไป production โดยไม่ต้องแก้ source:
...
Build มัน โดยให้ endpoint สาธารณะเดียวกันเป็น environment variable (SvelteKit อ่าน var ที่ขึ้นต้นด้วย `PUBLIC_` บน client — pattern จาก Module 11):
...
เสิร์ฟ `web/build` จาก Node host ตัวไหนก็ได้ (Fly.io, Render, VM) มัน sign user เข้าด้วย `supabase-js` และอ่านข้อมูลของพวกเขาผ่าน FastAPI ที่ deploy แล้วตัวเดียวกับที่ mobile app ใช้ — backend เดียว, client สองตัว, ตามที่สัญญาไว้เป๊ะ
...
แล้ว sign in บน client ตัวไหนก็ได้ (หรือ fetch token จาก Supabase Auth แบบ hosted) แล้วเรียก `/me` ด้วยมัน:
