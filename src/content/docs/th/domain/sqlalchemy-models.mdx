...
model พวกนี้คือฐานรากที่ทั้ง data layer ตั้งอยู่: [Schemas & repositories →](/fittrack/th/domain/schemas-and-repositories/) ห่อ model พวกนี้ใน Pydantic schema และ repository class และทุก API module ตั้งแต่ Exercises เป็นต้นไปก็อ่านและเขียนผ่าน model พวกนี้บน async session จาก [Async database →](/fittrack/th/fastapi/async-database/)
...
### เรื่องของ annotation ที่ใส่ไว้ในเครื่องหมายคำพูด

relationship ทุกตัวข้างบนเขียนเป็น string — `Mapped["Profile | None"]` — โดย import class จริงไว้ใต้ `if TYPE_CHECKING:` เท่านั้น ที่ต้องจับคู่กันแบบนี้เพราะ `exercise.py` กับ `profile.py` อ้างถึงกันและกัน ถ้า import ตรง ๆ ทั้งคู่ก็จะเป็น circular import type checker เลยได้ import ส่วน interpreter ได้ string ไป

Python 3.14 เปลี่ยนเรื่องนี้ไปครึ่งหนึ่ง ตาม [PEP 649](https://peps.python.org/pep-0649/) annotation จะไม่ถูก evaluate ตอนที่ class body ทำงานแล้ว แต่ถูกเก็บเป็น function และคำนวณเมื่อมีคนถามเท่านั้น (module `annotationlib` ตัวใหม่คือวิธีถามที่รองรับอย่างเป็นทางการ) ดังนั้น `Mapped[Profile | None]` แบบไม่มีเครื่องหมายคำพูดจะไม่โยน `NameError` ตอน import อีกต่อไป และ `from __future__ import annotations` ก็หมดหน้าที่ไป

สิ่งที่ *ไม่* เปลี่ยน: SQLAlchemy ยังต้อง resolve `Profile` ให้เป็น class จริงตอน configure mapper และ Pydantic ก็ทำแบบเดียวกันตอน build model — evaluate แบบ lazy ไม่ได้แปลว่าไม่ evaluate เลย ชื่อนั้นต้อง import ได้ ณ เวลาที่ resolve ซึ่งก็คือสิ่งที่ขั้นที่ 6 รับประกันให้ เพราะฉะนั้นคงรูปแบบ string ไว้ตรงนี้ดีกว่า: ไม่เสียอะไร ใช้ได้กับ 3.12 ด้วย และ `__init__.py` ข้างบนคือตัวที่ทำให้มัน resolve ได้จริง
...
`app/models/` คือภาพที่ typed ของสี่ Postgres table ของ FitTrack: `Base(DeclarativeBase)` บวก `Profile`, `Exercise`, `Workout`, และ `WorkoutSet` เขียนใน style `Mapped` / `mapped_column` ของ SQLAlchemy 2.0 ดังนั้น annotation ถือ nullability และ `mapped_column` ถือ key, foreign key, และ server default — ทุก column faithful กับ Supabase migration `relationship()` เชื่อมทั้งหมดเข้าด้วยกัน (`Workout.sets` ⇄ `WorkoutSet.workout` พร้อม `cascade="all, delete-orphan"`) ดังนั้น session พร้อม set ก่อตัวเป็น object graph เดียวที่คุณสร้างและลบเป็นหน่วยได้ การ import metadata แสดงสี่ table register พร้อม column type ที่ถูกต้อง model พวกนี้อยู่ลำพังก็ยังทำอะไรไม่ได้ — ต้องมี boundary ที่ validate แล้วสำหรับ input และ output และที่ทางสำหรับใส่ query ต่อไป [Schemas & repositories →](/fittrack/th/domain/schemas-and-repositories/) เพิ่ม Pydantic v2 schema และ async `ExerciseRepo` / `WorkoutRepo` ที่เปลี่ยน model พวกนี้เป็นชั้น read/write ที่ทุก API module เรียก
